...
import { MOCK_TRIPS, delay } from '../mock/data'; // fallback for mock data
        ...
        <div style={{ position: 'absolute', bottom: 48, left: 48, right: 48, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
          <div style={{ color: '#fff' }}>
            ...
            <div style={{ display: 'flex', gap: 24, fontSize: 16, opacity: 0.9 }}>
              <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}><MapPin size={18} /> {trip.destination}</span>
              <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}><Calendar size={18} /> {new Date(trip.startDate).toLocaleDateString()} - {new Date(trip.endDate).toLocaleDateString()}</span>
            </div>
          </div>
          <button 
            ...
            <>
              ...
              <div style={{ display: 'flex', gap: 12, overflowX: 'auto', paddingBottom: 16, marginBottom: 32, WebkitOverflowScrolling: 'touch' }}>
                {days.map((day, idx) => (
                  ...
                  >
                    <div style={{ fontSize: 13, opacity: activeDay === idx ? 1 : 0.7, marginBottom: 4 }}>Day {day.dayNumber}</div>
                    <div style={{ fontSize: 15, color: activeDay === idx ? 'var(--text-main)' : 'inherit' }}>{new Date(day.date).toLocaleDateString(undefined, { weekday: 'short', month: 'short', day: 'numeric' })}</div>
                  </button>
                ))}
              </div>
                        ...
                        <div style={{ flex: 1, background: '#fff', padding: 24, borderRadius: 'var(--r-xl)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)' }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}>
                            <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--primary)', display: 'flex', alignItems: 'center', gap: 8 }}>
                              {act.time} <span style={{ width: 4, height: 4, borderRadius: '50%', background: 'currentColor' }} /> {act.duration}
                            </div>
                            <div style={{ fontSize: 14, color: 'var(--text-muted)', fontWeight: 500 }}>{act.cost > 0 ? `${trip.currency}${act.cost}` : 'Free'}</div>
                          </div>
                          <h4 style={{ fontSize: 18, fontWeight: 700, color: 'var(--text-main)', marginBottom: 8 }}>{act.title}</h4>
                          <p style={{ fontSize: 15, color: 'var(--text-secondary)', lineHeight: 1.5 }}>{act.description}</p>
                        </div>
                      </div>
                    ))}
                    {currentDay.activities.length === 0 && (
                      <div style={{ padding: '32px', textAlign: 'center', color: 'var(--text-secondary)', background: '#fff', borderRadius: 'var(--r-xl)', border: '1px dashed var(--border)' }}>
